...
if len(sys.argv) != 2:
    print("Usage: dxlfiletransferservice <configuration files directory>")
    sys.exit(1)
...
logging_config_path = os.path.join(config_dir, FileTransferService.LOGGING_CONFIG_FILE)
...
with FileTransferService(sys.argv[1]) as app:
    try:
        # Run the application
        app.run()
        running = True
